package cn.ifengkou.athena.plugin;

import cn.ifengkou.athena.common.page.Page;
...
public class PagePlugin implements Interceptor {
    ...
    private static String pageSqlId = ""; //cn.ifengkou.athena.dal.mapper.xml中需要拦截的ID(正则匹配)
        ...
        if (ivk.getTarget() instanceof RoutingStatementHandler) {
            ...
            if (mappedStatement.getId().matches(pageSqlId)) { //拦截需要分页的SQL
                ...
                if (parameterObject == null) {
                    ...
                    //优化大数据的分页功能
                    String cSql = sql.substring(sql.indexOf("from"), sql.length());
                    //去掉排序，优化性能
                    //int endN = cSql.indexOf("order");
                    //if (endN != -1) {
                    //	 cSql = cSql.substring(0, endN);
                    //}
                    //String countSql = "select count(1) from (" + sql+ ") tmp_count"; //记录统计
                    String countSql = "select count(1) " + cSql;
                    PreparedStatement countStmt = connection.prepareStatement(countSql);
                    BoundSql countBS = new BoundSql(mappedStatement.getConfiguration(), countSql, boundSql.getParameterMappings(), parameterObject);
                    //下面这个循环用于修复在含有数组参数做foreach查询的时候得到不参数的BUG cjl -2014-12-02
...
//                         page = (Page) parameterObject;
    ...
    private String generatePageSql(String sql, Page page) {
        if (page != null && StringUtils.notEmpty(dialect)) {
            StringBuffer pageSql = new StringBuffer();
            if ("mysql".equals(dialect)) {
                pageSql.append(sql);
                if (page.getOrder() != null && !page.getOrder().equals("")) {//这个判断是用来排序的
